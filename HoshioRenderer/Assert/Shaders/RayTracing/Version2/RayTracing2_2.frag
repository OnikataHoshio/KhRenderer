...
layout(std430, binding = 0) buffer TriangleSSBO { Triangle Triangles[]; };
...
float rand() {
    uvec2 FragCoord = GetFragCoord();
    uint seed = uint(
        uint(FragCoord.x) * uint(1973) + 
        uint(FragCoord.y) * uint(9277) + 
        uint(uFrameCounter) * uint(26699)) 
        | uint(1);
...
vec3 PathTracing(Ray ray, int maxBounce)
{
    ...
    for(int i = 0; i < maxBounce; i++)
    {
        ...
        finalColor += vec3(material.Emissive) * throughput;

        vec3 wi = ToNormalHemisphere(SampleHemisphere(), hit_result.Normal);
        
        float PDF = 1.0 / (2.0 * PI); 
        float cosTheta = max(dot(wi, normalize(hit_result.Normal)), 0.0);
        vec3 brdf = vec3(material.BaseColor) / PI;

        throughput *= brdf * cosTheta / PDF;

        currRay.Start = hit_result.HitPoint + hit_result.Normal * 0.001;
...
void main()
{
    ...
    FragColor = vec4(PathTracing(ray, 8), 1.0);
}
